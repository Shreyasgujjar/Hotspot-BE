...
def resetConfig():
    ...
    for data in connectedDevices:
        if data != '':
            ...
            os.system("iptables -w -t filter -D vpnhotspot_acl -i " + wlan_details + " -s " + data.split(" ")[3].split("/")[0] + " -j ACCEPT")
    ...
    os.system("/system/bin/ip rule del  iif " + wlan_details + " lookup " + str(lookUpNumber) + " priority 17900")
    os.system("/system/bin/ip rule del  iif " + wlan_details + " unreachable priority 17980")
    os.system("iptables -w -t nat -D POSTROUTING -j vpnhotspot_masquerade")
